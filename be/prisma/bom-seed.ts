...
async function createBomSampleData() {
  console.log("🌱 Creating BOM sample data...");
  ...
  await prisma.$transaction(async (tx) => {
    ...
    console.log("🔄 Creating BOM version...");
    ...
    // 6. Create another BOM for Formal Shirt
    console.log("📋 Creating BOM for Formal Dress Shirt...");
    
    const formalShirtBom = await tx.bom.upsert({
      where: { code: "BOM-SH002-001" },
      update: {},
      create: {
        code: "BOM-SH002-001",
        productStyleId: formalShirt.id,
        name: "Formal Shirt BOM v1.0",
      },
    });

    console.log("  📝 Adding formal shirt BOM lines...");
    
    await tx.bomLine.create({
      data: {
        bomId: formalShirtBom.id,
        itemId: fabricItem.id,
        uom: "meter",
        qtyPerUnit: 1.5,
        wastagePercent: 3.0,
    ...
    // 7. Create customer and sales order
    ...
    console.log("✅ BOM sample data created successfully!");
    console.log("\n📊 Summary:");
    ...
    console.log(`- BOM Versions: ${await tx.bomVersion.count()}`);
...
createBomSampleData()
  .catch((e) => {
    console.error("❌ Failed to create BOM sample data:", e);
    process.exit(1);
  })
  .finally(async () => {
