import type { Request, Response, NextFunction } from "express";
import { ok } from "../../common/response";
import { SizesService } from "./sizes.service";

type VReq = Request & { validated?: any };

export class SizesController {
  static async list(req: VReq, res: Response, next: NextFunction) {
    try {
      return ok(res, await SizesService.list(req.validated.query));
    } catch (e) {
      return next(e);
    }
  }
  static async get(req: VReq, res: Response, next: NextFunction) {
    try {
      return ok(res, await SizesService.get(req.validated.params.id));
    } catch (e) {
      return next(e);
    }
  }
  static async create(req: VReq, res: Response, next: NextFunction) {
    try {
      return ok(res, await SizesService.create(req.validated.body));
    } catch (e) {
      return next(e);
    }
  }
  static async update(req: VReq, res: Response, next: NextFunction) {
    try {
      return ok(res, await SizesService.update(req.validated.params.id, req.validated.body));
    } catch (e) {
      return next(e);
    }
  }
  static async remove(req: VReq, res: Response, next: NextFunction) {
    try {
      return ok(res, await SizesService.remove(req.validated.params.id));
    } catch (e) {
      return next(e);
    }
  }
}
