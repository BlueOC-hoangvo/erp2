...
import { getSizes, createSize, updateSize, deleteSize } from "../../products/api/sizes.api";
import type { Size } from "../../products/api/sizes.api";
...
export default function Sizes() {
  ...
  const queryKey = useMemo(
    () => ["sizes", { page, pageSize, q, isActive }],
  ...
  const createMut = useMutation({
    mutationFn: (body: any) => createSize(body),
    onSuccess: async () => {
  ...
  const rows: Size[] = (data as any)?.data?.items ?? [];
...
function SizeFormModal({
  ...
  return (
    <Modal
      title={size ? "Sửa kích thước" : "Thêm kích thước mới"}
      open={visible}
      onOk={handleOk}
      onCancel={onCancel}
    ...
    >
      ...
      >
        <Form.Item
          ...
          label="Mã kích thước"
          rules={[
            { required: true, message: "Vui lòng nhập mã kích thước" },
            { max: 30, message: "Mã không được vượt quá 30 ký tự" },
          ]}
        >
          <Input placeholder="Nhập mã kích thước" />
        </Form.Item>

        <Form.Item
          name="name"
          label="Tên kích thước"
          rules={[{ max: 80, message: "Tên không được vượt quá 80 ký tự" }]}
        >
          <Input placeholder="Nhập tên kích thước (tùy chọn)" />
        </Form.Item>
